...
const HeaderSidebar = () => {
    ...
    return (
        <button
            ...
            className="flex w-full flex-row gap-[0.75rem] items-center p-[0.5rem] transition-all duration-300"
        >
            {isHovered ? (
                closeSidebar ? (
                    <div className="flex items-center justify-center w-[2rem] h-[2rem] rounded-full">
                ...
                <BeeMathLogo className="w-[2rem] h-[2rem]" />
            )}

            {!closeSidebar && (
                <p className="p-0 font-bevietnam text-[#090a0a] text-center text-2xl font-bold transition-all duration-300 hover:text-[#f97316]">
                    Toán thầy Bee
                </p>
